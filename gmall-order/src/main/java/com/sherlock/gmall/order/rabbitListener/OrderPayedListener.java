...
public class OrderPayedListener {
    ...
    /**
     * 只要我们收到支付宝给我回传的异步通知，告诉我们订单支付成功。返回并且必须 success 字符串，支付宝就不再通知
     * @param request
     * @return
     */
    ...
    public String handAliPayed(PayAsyncVo vo, HttpServletRequest request) throws AlipayApiException, UnsupportedEncodingException {
        ...
        log.info("支付宝成功之后传回的参数，{}", parameterMap);
        for (String key : parameterMap.keySet()) {
            log.info("请求参数：{}， 请求参数对应的值：{}", key, request.getParameter(key));
        }
	...
	*/
        if(signVerified) {//验证成功
            ...
            result = orderService.handPayResult(vo);

...
import com.sherlock.gmall.aop.LogAspects;
 ...
 *
 * 流程：
 * 		1）、传入配置类，创建ioc容器---------------------> 创建拦截普通bean创建的BeanPostProcessor
 ...
 * 		3）、调用AbstractApplicationContext.registerBeanPostProcessors(beanFactory);注册bean的后置处理器来方便拦截bean的创建；
 ...
 * 				创建internalAutoProxyCreator的BeanPostProcessor【{@link AnnotationAwareAspectJAutoProxyCreator} extends {@link BeanPostProcessor}】
 * 				@see AbstractBeanFactory#doGetBean(String, Class, Object[], boolean)
 * 					-> {@link AbstractBeanFactory#createBean(String, RootBeanDefinition, Object[])
 * 					   -> {@link AbstractAutowireCapableBeanFactory#createBean(String, RootBeanDefinition, Object[])}
 * 							-> {@link AbstractAutowireCapableBeanFactory#doCreateBean(String, org.springframework.beans.factory.support.RootBeanDefinition, Object[])}
 * 				1）、创建Bean的实例
 ...
 * 		4）、{@link AbstractApplicationContext#finishBeanFactoryInitialization(ConfigurableListableBeanFactory)}完成BeanFactory初始化工作；创建剩下的单实例bean
 * 			1）、遍历获取容器中所有的Bean，依次创建对象getBean(beanName);
 * 				getBean->doGetBean()->getSingleton()->
 * 			2）、创建bean
 * 			{@link AbstractAutoProxyCreator#postProcessBeforeInstantiation(Class, String)}
 * 				【{@link AnnotationAwareAspectJAutoProxyCreator }extends{@link BeanPostProcessor }在所有bean创建之前会有一个拦截，该类 extends {@link InstantiationAwareBeanPostProcessor }，会调用postProcessBeforeInstantiation()】
